...
public class ResultPage extends BasePage {
	...
	private static String[] freelingLanguages = {"en", "ca", "es", "pt", "it", "gl", "as", "cy"};
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					String language = Lang.detect(input);
					if (language.equals(danish)) {
						...
						if (!found) {
							System.out.println(">>> Unsupported language detected: " + language);
						}
					}
				} catch (Throwable t) {
					t.printStackTrace();
